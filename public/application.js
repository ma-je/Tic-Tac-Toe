...
var onSignUp = function onSignUp(event) {
  ...
  if (data.credentials.password === data.credentials.password_confirmation) {
    gameApi.addUser(data).then(gameUi.onSignUpSuccess).catch(gameUi.onError);
...
var onSignIn = function onSignIn(event) {
  ...
  gameApi.logInUser(data).then(gameUi.onsignInSuccess).catch(gameUi.onError);
};
...
var onSignOut = function onSignOut(event) {
  ...
  gameApi.logOut(data).then(gameUi.onSignOutSuccess).catch(gameUi.onSignUpError);
};
...
var onpasswordChange = function onpasswordChange(event) {
  ...
  gameApi.passwordChange(data).then(gameUi.resetSuccess).catch(gameUi.onError);
};

var onCreateGame = function onCreateGame(event) {
  gameApi.createGame().then(gameUi.onCreateGameSuccess).catch(gameUi.createGameError);
};
//
var onUpdateGame = function onUpdateGame(apiIndex, value, over) {
  ...
  gameApi.updateGame(apiIndex, value, over).then(gameUi.onUpdateGameSuccess).catch(gameUi.updateGameError);
};

var onGetGames = function onGetGames(event) {
  event.preventDefault();
  gameApi.index().then(gameUi.onGetSucess).catch(gameUi.onGetFailure);
};
module.exports = {
  onSignUp: onSignUp,
  onSignIn: onSignIn,
...
var gameEvents = __webpack_require__(1); // user require with a reference to bundle the file and use it in this file
...
function nextMove(square) {
  ...
  if (document.winner !== null) {
    ...
    // console.log(gameEvents)
    var _gameEvents = __webpack_require__(1);
    _gameEvents.onUpdateGame(apiIndex, value, over);
    //
    turnCount++;
    switchTurn();
  } else {
...
function checkForWinner(move) {
  var result = false; // assume it's false to start
...
function checkRow(a, b, c, move) {
  var result = false; // assume it is false
...
"use strict";
/* WEBPACK VAR INJECTION */(function($) {// example from class



var app = __webpack_require__(4);
var gameEvents = __webpack_require__(1);
var gamePlay = __webpack_require__(2);
...
var onGetSucess = function onGetSucess(data) {
  console.log(data);
  $('.gameStats').empty();
